...
type SqlCeConnector(connStr) = 
  let conn = new SqlCeConnection(connStr)
  let check() =
    try
      use comm = new SqlCeCommand("SELECT 1", conn)
  ...
  member this.ExecNonQuery s =
    check()
    let comm = new SqlCeCommand(s, conn)
  ...
  member this.ExecQuery (s:string, parms:seq<obj>, log) =
    ...
    let addParm (comm:SqlCeCommand) (idx:int) o =
      comm.Parameters.AddWithValue ("p__" + idx.ToString(), o) |> ignore
    seq {
      use comm = new SqlCeCommand(s, conn)
