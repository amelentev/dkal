...
type Engine =
  {
    ...
    mutable sqlCe : option<SqlCeConnector> // multipledatasources
    mutable dataSrc : option<DataSourceType> // multipledatasources
  ...
  static member Config (opts:Options) =
    let this =
      { me = None; worker = None;
        ...
        sentItems = dict()
        sql = None
        sqlCe = None // multipledatasources
        dataSrc = None // multipledatasources
  ...
  //multipledatasources    
  member this.DeriveSqlCe s (infon:Term) =
        let vars = (infon.Apply s).Vars()
        let checkAssumptions (augS:AugmentedSubst) =
          let apply (t:Term) = t.Apply augS.subst
          let sqlExpr = SqlCompiler.compile this.options this.NextId (augS.assumptions |> List.map apply)
          SqlCompiler.execSqlCeQuery (this.sqlCe.Value, this.Comm, this.options, sqlExpr, augS.subst, vars) |> Seq.toList   
        { substs = this.DoDerive [] (AugmentedSubst.NoAssumptions s) infon |> List.collect checkAssumptions }
  ...
  member private this.DoTalk () =
    ...
    if this.dataSrc=None || this.dataSrc=Some(DataSourceType.SqlServer) || this.dataSrc=Some(DataSourceType.SqlAzure) then //multipledatasources
        this.communications |> List.iter (fun comm -> (this.Derive Map.empty comm.trigger).All |> List.iter (runCommFor comm))
    elif this.dataSrc=Some(DataSourceType.SqlCe) then //multipledatasources
        this.communications |> List.iter (fun comm -> (this.DeriveSqlCe Map.empty comm.trigger).All |> List.iter (runCommFor comm))
  ...
  /// Reset method to select a DataSource  // multipledatasources
  member this.Reset datasrc =
    this.Close()
    this.dataSrc <- Some(datasrc)
    if datasrc=DataSourceType.SqlServer || datasrc=DataSourceType.SqlAzure then
        this.sql <- Some (SqlConnector this.options.PrivateSql)
    elif datasrc=DataSourceType.SqlCe then
        this.sqlCe <- Some (SqlCeConnector this.options.PrivateSql)
  ...
  member this.Ask (comm, i:Infon) =
    this.Invoke (fun () ->
      ...
      elif this.dataSrc=Some(DataSourceType.SqlCe) then
        this.Comm.QueryResults (i, (this.DeriveSqlCe Map.empty i).All |> Seq.map bind)
      this.Comm.RequestFinished ())
